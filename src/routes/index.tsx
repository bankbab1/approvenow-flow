...
function Index() {
  ...
  const addApprover = (id: number) =>
    setStages((s) =>
      s.map((st) =>
        st.id === id ? { ...st, approvers: [...st.approvers, people[0]] } : st,
      ),
    );
  ...
  const previewWorkflow = () => {
    ...
    stages.forEach((stage, index) => {
      ...
      if (stage.mode === "Single") {
        ...
        const required = stage.allRequired
    ...
    <div className="min-h-screen bg-muted p-8">
      <div className="mx-auto max-w-3xl">
        <div className="mb-5 rounded-2xl bg-card p-5 shadow-sm">
          <h2 className="mt-0 text-xl font-semibold text-card-foreground">
        ...
        <div>
          {stages.map((stage, index) => (
            <div
              key={stage.id}
              className="mb-5 rounded-2xl bg-card p-5 shadow-sm"
            >
              ...
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                <select
                  value={stage.mode}
                  onChange={(e) =>
                    setMode(stage.id, e.target.value as Mode)
                  }
                ...
                <div className="mb-3 flex flex-wrap items-center gap-3">
                  <label className="flex items-center gap-2 text-sm text-card-foreground">
                    <input
                      ...
                      onChange={(e) =>
                        updateStage(stage.id, { allRequired: e.target.checked })
                      }
                    />
              ...
              <h3 className="mt-0 mb-2 text-base font-semibold text-card-foreground">
                Approvers
              </h3>
              <div>
                {stage.approvers.map((appr, idx) => (
                  <div key={idx} className="mb-2 flex gap-2">
                    <select
                      value={appr}
                      onChange={(e) =>
                        setApprover(stage.id, idx, e.target.value)
                      }
                      className="flex-1 rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring"
                    >
                      {people.map((p) => (
        ...
        <div className="mb-5 rounded-2xl bg-card p-5 shadow-sm">
          <h2 className="mt-0 mb-3 text-xl font-semibold text-card-foreground">
            Workflow Preview
          </h2>
